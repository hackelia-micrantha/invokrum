...
| T12 | Parser, dependency, build, release, or artifact compromise changes behavior. | Partial | The project toolchain and Cargo graph are pinned; external actions use immutable commits; RustSec advisories, licenses, dependency sources, wildcard constraints, and full Git history secrets are blocking gates; Linux, Intel macOS, and Windows release targets build in CI; tagged prereleases publish deterministic archives, SHA-256 checksums, SPDX SBOMs, and keyless provenance/SBOM attestations only after reusable CI succeeds. | GitHub-hosted runners and images, registries, scanner toolchains, CI identity infrastructure, and dependency distribution remain external trust; maintain narrow reviewed exceptions and verify published artifacts. |
